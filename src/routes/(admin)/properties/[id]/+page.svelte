...
<article class="rounded bg-white/70 max-w-3xl p-2">
	...
	<section class="flex flex-col pr-5 justify-between lg:flex-row">
		...
		<figure class="h-fit w-64">
			<Image
				src={String(property?.agent.avatar)}
				alt={property?.agent.name}
				class="rounded-full mx-auto w-32"
			/>
			<figcaption class="mt-2 text-center">
				<h4 class="font-semibold text-lg text-gray-700">{property?.agent.name}</h4>
				<p class="text-sm text-teal-700">{property?.agent.email}</p>
