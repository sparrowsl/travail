	...
	<figcaption>
		...
		<div class="flex flex-wrap mt-3 gap-5">
			...
			<p class="flex text-sm text-gray-700 gap-1 items-center">
				...
				<span>{agent?.location || "Los Angeles"}</span>
			</p>
			<p class="flex text-sm text-gray-700 gap-1 items-center">
				<i class="text-xl i-mdi:phone-outline" />
				<span>{agent?.contact || "+232-99-905-600"}</span>
			</p>
			<p class="flex text-sm text-gray-700 gap-1 items-center">
